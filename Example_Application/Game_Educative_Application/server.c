        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				...
				// Si le nombre de joueurs atteint 4, alors on peut lancer le jeu
                                ...
                                if (nbClients==4)
				{
					// On envoie les cartes aux joueurs 0,1,2,3, ainsi que la ligne qui leur correspond dans tableCartes
					for (int i = 0; i < 4; i++) 
					{
						// On envoie les 3 cartes du joueur i
						sprintf(reply, "D %d %d %d", deck[i*3], deck[i*3+1], deck[i*3+2]);
						sendMessageToClient(tcpClients[i].ipAddress, tcpClients[i].port, reply);

						// On envoie la ligne correspondante de tableCartes pour le joueur i
						for (int j = 0; j < 8; j++) 
						{
...
// With the help mentioned in the README and the code base provided in the instructions