		...
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					// Asking O_bject to every client buffer[0]=O
					}
					else if ((objetSel!=-1) && (joueurSel!=-1))
					{
						sprintf(sendBuffer,"S %d %d %d",gId, joueurSel,objetSel);
					// Asking S_elecion from specific client buffer[0]=S
					}
					// Envoi du message si un des cas a rempli le buffer same commande if G_uilty / O_bject / S_elecion
    				sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
				}
				else
				{
